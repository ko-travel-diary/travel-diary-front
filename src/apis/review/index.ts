...
import { GetTravelReviewBoardResponseDto, GetTravelReviewCommentListResponseDto, GetTravelReviewDetailResponseDto, GetTravelReviewMyListResponseDto, GetReviewTitleAndContentSearchRequestDto, GetReviewWriterSearchRequestDto, GetReviewWriteDateSearchRequestDto, GetTravelReviewFavoriteStatusResponseDto } from "./dto/response";
...
export const increaseViewCountRequest = async(reviewNumber: number | string) => {
    const result = await axios.patch(PATCH_INCREASE_VIEW_COUNT_REQUEST_URL(reviewNumber))
        .then(requestHandler<ResponseDto>)
        .catch(requestErrorHandler);
    return result;
}
